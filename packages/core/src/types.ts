...
export type CreateCurrentTarget<Events, Element extends Atomico<any>> = {
  [I in keyof Events]?: Events[I] extends (
    event: CustomEvent<infer Detail>
  ) => any
    ? (
...
export type CreateWrapper<Element extends AtomicoELement> =
  HTMLAttributes<HTMLElement> & {
    ...
    ref?: Ref<HTMLElement>;
    key?: Key;
  } & (Element extends Atomico<infer Config>
      ? Partial<
          Omit<
            InferProps<Config["props"]>,
            KeyofPropsWithEvents<Config["props"]>
          >
        > &
