...
const createWrapper = ({ createElement, useLayoutEffect, forwardRef, useRef }) => (tagName, base, { extends: is } = {}) => forwardRef(
  ({ children, ...props }, parentRef) => {
    ...
    for (let prop in props) {
      ...
      if (prop.startsWith("on") && value == null || typeof value == "function") {
        let timeStamp;
        const handler = (event) => {
